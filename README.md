...
Type `make -j` to install the software. The makefile automatically compiles the program. Due to limitations of GNU Make, the path of PECA is not allowed to contain any whitespace characters. The PECA software consists of the Markov chain Monte Carlo (MCMC) sampler and a Python script to summarize the results from the MCMC samples. The Python script requires [Python 3.x](https://www.python.org/) and [matplotlib](https://matplotlib.org/).

### Windows

Executables for 64-bit Windows are included [here](https://github.com/PECAplus/PECAplus_cmd_line/tree/master/win64bin).
