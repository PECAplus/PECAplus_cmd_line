...
void print_data(const Pre& pr,const string& str0)
{
    ...
    {
        ofstream ofs((str0+"y.txt").c_str());//output normalized data
        ofs<<pr.yh().at(0);
        for (int l=1; l<pr.op().nc()+pr.op().level(); l++) ofs<<'\t'<<pr.yh().at(l);
        ofs<<'\n';
        for (int p=0; p<pr.nprot(); p++) {
            for (unsigned q=0; q<pr.yy().at(p).size(); q++) {
                ...
                for (int l=0; l<pr.op().nc(); l++) {
                    if (obs(pr.yy().at(p).at(q).at(l))) ofs<<'\t'<<pr.yy().at(p).at(q).at(l);
                    else ofs<<"\tNA";
                }
                ofs<<'\n';
            }
